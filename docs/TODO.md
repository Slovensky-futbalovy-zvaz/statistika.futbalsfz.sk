...
- [~] Frontend (ADR-0005 — Astro + ECharts + Tailwind): **hotové** — mapa SR, profil zväzu, per-sezónne stránky, graf kategórií, pohlavie+osoby, medziročné porovnanie KPI, F4 porovnanie a radenie zväzov, **F5 demografia** (10-ročné trendy osôb po rolách na profile). Build 646 stránok. **Ďalej:** vlastná doména (viď nižšie).
- [x] **Nasadenie na Cloudflare — Workers static assets** (19. 7. 2026) — Cloudflare medzičasom zlúčil Pages do Workers; nasadené cez git-connected Worker (`web/wrangler.jsonc`, Root directory `web`, build `pnpm install && pnpm build`, deploy `npx wrangler deploy`). Prvý deploy úspešný: 657 stránok, 664 assetov nahraných, **https://statistika-futbalsfz-sk.jan-letko.workers.dev** live a overený (profil SFZ, futsal, demografia, pyramída). Postup zdokumentovaný v `docs/cloudflare-pages-nasadenie.md`. **Zostáva:** pridať vlastnú doménu `statistika.futbalsfz.sk` (Settings → Domains & Routes → Custom Domain).
